...
static void printUsage(const char* prog) {
    fprintf(stderr,
            ...
            "  on            Turn fan fully on\n"
            "  off           Turn fan off\n"
            "  speed <N>     Set fan speed to N%% (0-100)\n"
...
int main(int argc, char* argv[]) {
    ...
    if (cmd == "temp") {
        ...
        if (!status.isOk()) { fprintf(stderr, "ERROR: %s\n", status.getDescription().c_str()); return 1; }
        ...
        if (!status.isOk()) { fprintf(stderr, "ERROR: %s\n", status.getDescription().c_str()); return 1; }
        if (rpm < 0) printf("Fan RPM: N/A (tachometer not wired)\n");
        else printf("Fan RPM: %d\n", rpm);
        ...
        if (argc < 3) { fprintf(stderr, "ERROR: 'speed' requires a percentage argument (0-100)\n"); return 1; }
        int pct = atoi(argv[2]);
        ...
        if (!status.isOk()) { fprintf(stderr, "ERROR: %s\n", status.getDescription().c_str()); return 1; }
        printf("Fan returned to AUTO mode\n");
